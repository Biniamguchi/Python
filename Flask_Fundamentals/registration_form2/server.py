...
def form():
    ...
    if len(request.form['first']) < 1:
        ...
        flash(u"First name should only include letters.", 'first')
    ...
    if len(request.form['last']) < 1:
        ...
        flash(u"First name should only include letters.", 'last')
        session['flash_count'] += 1
        session['last'] = ""
    else:
    ...
    if len(request.form['password']) < 8 or not request.form['password'].istitle() or request.form['password'].isalpha():
        flash(u'Password must be at least 9 characters with at least one uppercase letter and one number.', 'password')
        session['flash_count'] += 1
        session['password'] = ""
    else:
    ...
    if not request.form['confirm'] == request.form['password']:
        flash(u'Password confirmation does not match your initial password. Please re-enter both your password and confirmation.', 'confirm')
        session['flash_count'] += 1
        session['password'] = ""
        session['confirm'] = ""
    else:
    ...
    if session['flash_count'] > 0:
        ...
        flash('Registration is complete! Thanks for signing up!', 'submit')
